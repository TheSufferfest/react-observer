...
const buildNode = (stats_user, render_stat, stats_DOM) => {
  if (is_debug) {
    log('render time (event: ' + stats_user.type + '): ' + (render_stat.start_time - stats_user.start_time), 'paint time: ' + (stats_DOM.start_time - render_stat.start_time));
  }
  ...
  let line = createComplexElement(render_stat.component_name, render_time + 'ms', paint_time + 'ms', total_time + 'ms', is_render_good, is_paint_good);
...
const buildPaint = (stats_user, stats_DOM) => {
  ...
  let line = createComplexElement('unknown', total_time + 'ms', is_total_good);
